...
export async function getSanityPage(slug, language = 'en') {
  ...
  const slugMap = {
    ...
    'insurance-consulting': 'insurance-consulting',
  ...
  let finalSlug = slugMap[slug] || slug

  // Try primary slug
  let page = await getPageBySlug(finalSlug, language)

  // Fallback: for blog detail pages where the file path slug doesn't include "blog-"
  // e.g. path: "retirement-planning-in-your-30s" → Sanity slug: "blog-retirement-planning-in-your-30s"
  if (!page && finalSlug && !finalSlug.startsWith('blog-')) {
    const blogSlug = `blog-${finalSlug}`
    page = await getPageBySlug(blogSlug, language)
  }
