...
export async function getPageBySlug(slug, language = 'en') {
  try {
    const query = `*[_type == "page" && slug.current == $slug && language == $language][0]{
      ...,
      relatedPosts[]->{
        _id, title, slug, text, publishedAt, coverImage, tag
      }
    }`
...
// Fetch the home page singleton for a given language
export async function getHomePage(language = 'en') {
  try {
    const id = `homePage-${language}`
    const query = `*[_type == "homePage" && _id == $id][0]`
    const page = await client.fetch(query, { id })
    return page || null
  } catch (error) {
...
// Fetch the About Page singleton for a given language
export async function getAboutPage(language = 'en') {
  try {
    const id = `aboutPage-${language}`
    const query = `*[_type == "aboutPage" && _id == $id][0]{
      pageTitle,
      intro,
      language,
      founders[]{
        ...
        title,
        intro,
        photo{ asset->{url}, alt, hotspot, crop }
      }
    }`
    const page = await client.fetch(query, { id })
    return page || null
